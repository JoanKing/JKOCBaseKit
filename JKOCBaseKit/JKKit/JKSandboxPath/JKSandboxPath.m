...
#pragma mark -- 数据归档 解档 删除
// 把对象归档存到沙盒里
+ (BOOL)jk_saveArchiveObject:(id)object byFileName:(NSString *)fileName {
    NSString *path  = [self jKCreateFile:fileName];
...
// 通过文件名从沙盒中找到归档的对象
+ (id)jk_getArchiveObjectByFileName:(NSString *)fileName {
    NSString *path  = [self jKCreateFile:fileName];
    path = [path stringByAppendingString:@".archive"];
...
// 根据文件名删除沙盒中的文件
+ (void)jk_removeArchiveObjectByFileName:(NSString *)fileName {
    NSString *path  = [self jKCreateFile:fileName];
    path = [path stringByAppendingString:@".archive"];
